...
void tickAutoAnswer(string win){
    ...
    action << "DISPLAY=:1 xdotool mousemove --sync --window " << win << " 40 60";
    ...
    action << "DISPLAY=:1 xdotool mousemove --sync --window " << win << " 210 261";
    xdo_result = system(action.str().c_str());
    action.str(std::string());
    ...
    action << "DISPLAY=:1 xdotool mousemove --sync --window " << win << " 640 440";
    xdo_result = system(action.str().c_str());
    action.str(std::string());
    ...
    action << "DISPLAY=:1 xdotool mousemove --sync --window " << win << " 820 440";
    xdo_result = system(action.str().c_str());
    action.str(std::string());
...
void autoAnswer(string username){
    FILE *fpipe;
    char line[512];   
    ...
    while ( fgets( line2, sizeof line2, fpipe2)) {
     if(strstr(line2,windowName.str().c_str())){
        ...
        cout << "Window: " << win << endl;
        tickAutoAnswer(win);
        cout << "(Un)Ticked AutoAnswer Box" << endl;
        break;
     }
    }
    pclose(fpipe2);
